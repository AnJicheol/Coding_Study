...
스택을 이용한 풀이는 스택을 전혀 이해하지 못하고 사용한거 같다
해당 알고리즘은 처음알고리즘에 배열을 스택으로 교체한 것인데
성능은 5~6배 느려지지만 이로 얻는 자료구조에서의 이점을
얻지 못했다 말 그대로 최악의 알고리즘이다

자료구조는 만능이 아닌 적재적소 활용해야 빛을 발한다
그저 스택으로 구현하는 알고리즘 문제이기 때문에 스택을
사용하는 것은 반성해야한다

자료구조 공부가 더 필요해보인다 스택을 이해하지 못하면
피드백을 할 수가 없다 
